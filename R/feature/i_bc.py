...
def i_bc(log_x, split_point, start_date, end_date):
    log_bc = log_x[(log_x.behavior_type==4) & \
               (log_x.date_time<=split_point-start_date+1) & \
                (log_x.date_time>=split_point-end_date+1)]
    feature_bc = log_bc.groupby(['item_id']).count().reset_index() \
        [['item_id', 'user_id']]
    fname = name.feature_name('bc', 'i', start_date, end_date)
    filename = name.file_name(split_point, fname)
    feature_bc.columns = [u'item_id', fname]
...
if __name__ == "__main__":
    ...
    for split_point in split_point_list:
        ...
        for (start_date, end_date) in bc_intervals:
            i_bc(log_x, split_point, start_date, end_date)
